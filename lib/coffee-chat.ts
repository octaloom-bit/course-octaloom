...
- מותר ניסוח היפותטי שבודק מה חסר, כל עוד הוא נשאר היפותטי ואין בו בקשה בפועל. למשל "אם היית ממליץ עליי למישהו מחר, מה היית צריך לדעת עליי קודם?" מותר, כי הוא שואל מה חסר. "תמליץ עליי?" אסור.
- החריג השני הוא חלק ה-avoid, שבו כן מנסחים בדיוק את הבקשה האסורה כדי להזהיר ממנה.`;
...
- רשימת מילים אסורות, בכל הטיה ובכל מגדר, גם בתוך ציטוט בחלק ה-avoid: רלוונטי, מגוון, מרתק, חיוני, מהותי, ייחודי, מקיף, חדשני, פורץ דרך, משמעותי, מאתגר, רב-ממדי, חסר תקדים.
- בלי סופרלטיבים ריקים ("מדהים", "לייצר ערך", "לקחת לשלב הבא") ובלי קלישאות לינקדאין.
