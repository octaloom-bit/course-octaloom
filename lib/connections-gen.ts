...
export const SCENARIOS: Scenario[] = [
  ...
  {
    id: "insider-open",
    title: "פנייה ראשונה לעובד בחברה שאתם רוצים להיכנס אליה",
    when: "ראיתם משרה. במקום לפנות למגייסת, פונים למישהו מהצוות עצמו. זו רק פתיחת שיחה, בלי לבקש כלום.",
    kind: "request",
    anchor:
  ...
  {
    id: "referral-ask",
    title: "בקשת הפניה למשרה, אחרי שכבר יש שיחה",
    when: "רק אחרי שהייתה שיחה אמיתית, ורק אם זה מרגיש טבעי. לא כהודעה ראשונה.",
    kind: "message",
    anchor:
...
- עדיף 150-250 תווים. קצר ואנושי מנצח ארוך ומנומק.`
  ...
  const agendaRule =
    s.id === "ifp"
      ...
      : s.id === "referral-ask"
      ? `- כאן מותר לבקש, וזו כל המטרה: לבקש להגיש מועמדות דרך האדם הזה.
- הבקשה היא משפט אחד בסוף, אחרי משפט אחד של הקשר מהשיחה שכבר הייתה. סך הכל שני משפטים.
- הניסוח חייב להשאיר לו דלת יציאה נוחה: "אם זה מתאים לך", "אם זה לא מסובך". בלי לחץ ובלי רגשות אשם.
- אסור לצרף קורות חיים בהודעה הזאת, אסור לינק, ואסור לפרט למה אתם מתאימים. זה נשמר לשיחה עצמה.
- אסור לכתוב שהוא יקבל בונוס. הוא יודע.`
      : s.id === "insider-open"
      ? `- זו פתיחת שיחה בלבד. אסור להזכיר את המשרה, אסור לרמוז שאתם מחפשים עבודה, ואסור לבקש שום דבר.
- ההודעה נגמרת בשאלה אמיתית שהוא ישמח לענות עליה, כזאת שנובעת מהחומר.
- אם זה יישמע כמו פנייה מחופשת לבקשת עבודה, נכשלנו.`
